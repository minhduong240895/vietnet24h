...
class BaseRepository extends Repository implements RepositoryInterface
{
     ...
     * @return string
     */
    public function saveImage($image, $directory = '', $watermark = 'true', $size = ['width' => 800, 'height' => 600])
    {
        try {
...
//                }
            ...
            if($watermark == true){
                $watermark = Image::make(dirname(base_path()).'/assets/img/watermark.png');
                $img->insert($watermark, 'top-right', 0, 0);
            }
            $path = $pathToDirectory . $thumbName;

            $img->save(dirname(base_path()).'/' . $path);
